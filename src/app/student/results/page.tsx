...
interface ExamResult {
  ...
  grade?: string // Database stored grade - we ignore this
}
...
// ─── Grading ─────────────────────────────────────────
// ✅ WAEC Grading Scale - RECALCULATED in UI
    ...
    >
      <div className="relative bg-white rounded-xl border border-slate-200/60 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200 overflow-hidden group h-full">
        ...
        <div className="p-4">
          ...
          <p className={cn('text-xl sm:text-2xl font-black leading-none tracking-tight', t.valueColor)}>{value}</p>
          {subtitle && (
...
// ═══════════════════════════════════════════════════
// ✅ SUBJECT PERFORMANCE CARD - Uses recalculated grade
// ═══════════════════════════════════════════════════
        ...
        <div className="p-3.5">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex-1 min-w-0">
              <p className="font-bold text-sm text-slate-900 leading-tight truncate">{perf.subject}</p>
            ...
            >
              <span className={cn('text-[10px] font-black', g.text)}>{grade}</span>
            </div>
          </div>

          <div className="flex items-baseline gap-1 mb-2">
            <span className={cn('text-xl font-black leading-none tracking-tight', g.text)}>{perf.averageScore}</span>
          ...
          <div className="flex items-center justify-between text-[10px] font-semibold">
            ...
            {perf.examsTaken > 1 && (
              <div className="flex items-center gap-1 text-slate-400">
                ...
                <span className="text-[9px]">{Math.round(perf.bestScore - perf.lowestScore)}%</span>
              </div>
            )}
          </div>
        </div>
...
// ═══════════════════════════════════════════════════
// ✅ RESULT ROW - FIXED: Uses recalculated grade, NOT database grade
// ═══════════════════════════════════════════════════
function ResultRow({ result, index, onClick }: { result: ExamResult; index: number; onClick: () => void }) {
  // ✅ FORCE recalculation from percentage - IGNORE database grade
  const percentage = Number(result.percentage) || 0
  const grade = getWAECGrade(percentage) // This gives A1, B2, C6, D7, etc.
        ...
        <div className="flex items-stretch">
          ...
          <div className="flex-1 p-4 min-w-0">
            <div className="flex items-start gap-3">
              {/* ✅ GRADE BADGE - Uses recalculated grade, NOT result.grade */}
              <div className="shrink-0">
                ...
                >
                  <span className={cn('text-[9px] font-bold uppercase leading-none opacity-70', g.text)}>Grade</span>
                  <span className={cn('text-base font-black leading-none mt-0.5', g.text)}>{grade}</span>
                </div>
              </div>

              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-sm text-slate-900 truncate leading-snug mb-1.5">{result.exam_title}</h3>
              ...
              <div className="shrink-0 text-right flex flex-col items-end gap-1">
                <div className="flex items-baseline gap-0.5">
                  <span className={cn('text-xl font-black leading-none', g.text)}>{percentage}</span>
...
export default function StudentResultsPage() {
    ...
    async (showToast = false) => {
      ...
      try {
        ...
        for (const attempt of attemptsData) {
          ...
          // ✅ RECALCULATE - IGNORE database grade
          ...
          completedResults.push({
            ...
            grade: getWAECGrade(percentage), // ✅ Recalculated - not stored
          })
        ...
        // Subject Performance with recalculated grades
  ...
  useEffect(() => {
    ...
    const ch1 = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'exam_attempts', filter: `student_id=eq.${profile.id}` },
        () => loadResults(true)
      )
      .subscribe()
    const ch2 = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'ca_scores', filter: `student_id=eq.${profile.id}` },
        () => loadResults(true)
      )
      .subscribe()
    return () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      f = f.filter(
        (r) => r.exam_title.toLowerCase().includes(q) || r.exam_subject.toLowerCase().includes(q)
      )
    }
    switch (sortOrder) {
      case 'newest':
        f.sort((a, b) => new Date(b.completed_at || '').getTime() - new Date(a.completed_at || '').getTime())
        break
      case 'oldest':
        f.sort((a, b) => new Date(a.completed_at || '').getTime() - new Date(b.completed_at || '').getTime())
        break
      case 'highest':
                ...
                <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2.5">
                  <div className="flex items-center justify-between px-1 mb-2">
                    <p className="text-xs text-slate-500 font-semibold">
                      Showing <span className="font-black text-slate-800">{filteredResults.length}</span> result
