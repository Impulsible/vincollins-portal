// app/staff/exams/[examId]/preview/page.tsx
'use client'
...
import { 
  ...
  Flag, Shield, FileText, Brain, AlertCircle, Home
} from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center py-12 px-4">
          ...
          <Button onClick={() => router.push('/staff/exams')} className="mt-4">
            Back to Exams
          </Button>
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 md:px-5 lg:px-6 py-3 sm:py-4 space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <Button variant="ghost" size="icon" onClick={() => router.push(`/staff/exams/${examId}`)} className="h-8 w-8 sm:h-9 sm:w-9 shrink-0">
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-base sm:text-lg md:text-xl font-bold">Exam Preview</h1>
                <Badge variant="outline" className="text-[10px] sm:text-xs">Preview Mode</Badge>
              </div>
              <p className="text-xs sm:text-sm text-muted-foreground">{exam.title} • {exam.subject} • {exam.class}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge className={cn("text-[10px] sm:text-xs", getTimerColor())}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          ...
          <Card>
            <CardContent className="p-2.5 sm:p-3 text-center">
              <XCircle className="h-4 w-4 sm:h-5 sm:w-5 text-orange-600 mx-auto mb-1" />
        ...
        <div className="bg-white rounded-lg border p-2 sm:p-3">
          ...
          <Progress value={((currentIndex + 1) / totalQuestions) * 100} className="h-1.5 sm:h-2" />
        </div>
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-7 h-7 sm:w-8 sm:h-8 rounded-lg text-[10px] sm:text-xs font-medium transition-all flex-shrink-0",
        ...
        {/* Question Content */}
        <div className="bg-white rounded-lg border p-3 sm:p-4 lg:p-6 min-h-[300px] sm:min-h-[350px]">
          {currentQuestion && (
            <div className="max-w-3xl mx-auto">
              ...
              <h4 className="text-sm sm:text-base lg:text-lg font-medium mb-4 sm:mb-6 break-words">
                  ...
                  {currentQuestion.options?.map((option, idx) => (
                    ...
                    >
                      ...
                      <Label 
                        ...
                        className="flex-1 cursor-pointer text-xs sm:text-sm break-words"
                      >
        ...
        <div className="flex justify-between items-center gap-2">
          ...
          <div className="text-[10px] sm:text-xs text-muted-foreground flex items-center gap-1">
            ...
            <span className="hidden xs:inline">Question</span> {currentIndex + 1} of {totalQuestions}
          </div>
        ...
        {exam.instructions && (
          <div className="bg-blue-50 rounded-lg p-3 sm:p-4 border border-blue-200">
            ...
            <p className="text-[10px] sm:text-xs text-blue-700 whitespace-pre-line">{exam.instructions}</p>
          </div>
        )}
        ...
        <div className="bg-slate-50 rounded-lg p-3 sm:p-4">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 text-[11px] sm:text-sm">
            <div>
