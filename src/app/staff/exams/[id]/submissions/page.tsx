      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-3 sm:p-4">
            ...
            <p className="text-lg font-bold">{submissions.filter(s => s.status === 'completed').length}</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-3 sm:p-4">
            <p className="text-[11px] text-slate-500">Graded</p>
            <p className="text-lg font-bold text-purple-600">{submissions.filter(s => s.status === 'graded').length}</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-3 sm:p-4">
            <p className="text-[11px] text-slate-500">Pending</p>
            <p className="text-lg font-bold text-amber-600">{submissions.filter(s => s.status === 'pending_theory' || s.status === 'completed').length}</p>
          </CardContent>
        </Card>
      </div>
                    ...
                    <TableRow key={submission.id} className="hover:bg-slate-50">
                      ...
                      <TableCell className="text-center">
                        <span className={cn("font-semibold text-xs sm:text-sm", 
                          (submission.percentage || 0) >= 50 ? 'text-emerald-600' : 'text-red-600')}>
                          {submission.total_score || 0}
                        </span>
                      </TableCell>
                      <TableCell className="text-center text-xs sm:text-sm">{submission.percentage || 0}%</TableCell>
                      <TableCell className="text-center">
                        <Badge className={cn("text-xs", 
                          (submission.percentage || 0) >= 75 ? 'bg-emerald-100 text-emerald-700' :
                          (submission.percentage || 0) >= 50 ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700')}>
                          {getWAECGrade(submission.percentage || 0)}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center">{getStatusBadge(submission.status)}</TableCell>
                      <TableCell className="text-center text-[10px] sm:text-xs text-slate-500">{formatDate(submission.submitted_at)}</TableCell>
