// app/admin/notifications/page.tsx - FULLY RESPONSIVE WITH PROPER SPACING
'use client'
...
import { Loader2, CheckCircle, Award, BookOpen, Bell, Trash2, ChevronRight, Home, FileText } from 'lucide-react'
...
interface Notification {
  ...
  title: string
  message: string
  type?: 'exam_graded' | 'new_exam' | 'needs_grading' | 'exam_submitted' | 'general'
  read: boolean
  created_at: string
  link?: string
}
...
export default function AdminNotificationsPage() {
  ...
  const getNotificationIcon = (type?: string) => {
    switch (type) {
      ...
      case 'exam_submitted':
        return { icon: FileText, color: 'text-purple-600', bgColor: 'bg-purple-100', label: 'Exam Submitted' }
      case 'needs_grading':
  ...
  const formatProfileForHeader = (profile: Profile | null): HeaderUser | undefined => {
    ...
    if (!fullName) {
      firstName = profile?.email?.split('@')[0] || 'Admin'
      fullName = firstName
    }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
                    Exam submissions, approvals, and system updates
                  </p>
                </div>
              </div>
