...
export function ExamTabs({ activeTab, onTabChange, availableCount, upcomingCount, completedCount }: ExamTabsProps) {
  const counts = { available: availableCount, upcoming: upcomingCount, completed: completedCount }

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-5">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'
              )}
            >
              <span className={cn('h-1.5 w-1.5 rounded-full shrink-0', isActive ? 'bg-white/60' : tab.dot)} />
