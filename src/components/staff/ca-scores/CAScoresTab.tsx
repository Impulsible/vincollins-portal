// src/components/staff/ca-scores/index.tsx
// ============================================
...
import { 
  ...
  Loader2, Filter, Download, RefreshCw, XCircle, FileText
} from 'lucide-react'
...
interface CAScore {
  ...
  exam_objective_score: number | null
  exam_theory_score: number | null
...
// HELPER FUNCTIONS
// ============================================
const calculateTotal = (ca1: number, ca2: number, objective: number, theory: number): number => {
  return (ca1 || 0) + (ca2 || 0) + (objective || 0) + (theory || 0)
}
...
export function CAScoresTab({ staffProfile, termInfo }: CAScoresTabProps) {
  ...
  useEffect(() => {
    if (selectedClass && selectedSubject && selectedExamId) {
      loadStudents()
      loadExistingScores()
    }
  }, [selectedClass, selectedSubject, selectedTerm, selectedYear, selectedExamId])
  ...
  const loadExamsForSubject = async () => {
    try {
      const { data, error } = await supabase
        .from('exams')
        .select('id, title, subject, term, session_year')
  ...
  const loadStudents = async () => {
    try {
      ...
      formatted.forEach(student => {
        entries[student.id] = { ca1: '', ca2: '', objective: '', theory: '', remark: '' }
      })
      ...
      data.forEach((score: CAScore) => {
        if (entries[score.student_id]) {
          entries[score.student_id] = {
            ...
            objective: score.exam_objective_score?.toString() || '',
            theory: score.exam_theory_score?.toString() || '',
            remark: score.remark || ''
          }
        }
  ...
  // Auto-fetch exam scores from exam_attempts
  const fetchExamScores = async (studentId: string) => {
    if (!selectedExamId) return { objective: 0, theory: null }
    ...
    try {
      ...
      if (data) {
        ...
        if (data.theory_feedback) {
          const feedback = typeof data.theory_feedback === 'string' 
            ? JSON.parse(data.theory_feedback) 
            : data.theory_feedback
        ...
        return {
          objective: Number(data.objective_score) || 0,
          theory: theoryScore
        }
      }
    } catch (error) {
    ...
    return { objective: 0, theory: null }
  }
  ...
  const handleScoreChange = (studentId: string, field: string, value: string) => {
    const maxValues: Record<string, number> = {
      ca1: MAX_SCORES.ca1,
      ca2: MAX_SCORES.ca2,
      objective: MAX_SCORES.objective,
      theory: MAX_SCORES.theory
    }
    
    let numValue = parseFloat(value) || 0
    const maxValue = maxValues[field] || 100
  ...
  const handleAutoFetchScores = async (studentId: string) => {
    const scores = await fetchExamScores(studentId)
    setScoreEntries(prev => ({
      ...prev,
      [studentId]: {
        ...prev[studentId],
        objective: scores.objective.toString(),
        theory: scores.theory !== null ? scores.theory.toString() : prev[studentId]?.theory || ''
      }
    }))
    toast.success('Exam scores auto-fetched')
  }
  ...
  const handleSaveScores = async () => {
    ...
    try {
      ...
      for (const student of students) {
        ...
        if (ca1 === 0 && ca2 === 0 && objective === 0 && theory === 0) continue
        
        const total = calculateTotal(ca1, ca2, objective, theory)
        ...
        scoresToUpsert.push({
          ...
          ca1_score: ca1,
          ca2_score: ca2,
          exam_objective_score: objective,
          exam_theory_score: theory,
          total_score: total,
          grade: grade,
  ...
  const handleExport = () => {
    const csvData = students.map(student => {
      ...
      const objective = parseFloat(entry?.objective) || 0
      const theory = parseFloat(entry?.theory) || 0
      const total = ca1 + ca2 + objective + theory
    ...
    a.href = url
    a.download = `ca_scores_${selectedClass}_${selectedSubject}_${selectedTerm}.csv`
    a.click()
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-1.5">
              <Badge variant="outline" className="bg-white text-xs">CA1</Badge>
              <span className="text-xs">/20</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Badge variant="outline" className="bg-white text-xs">CA2</Badge>
              <span className="text-xs">/20</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Badge variant="outline" className="bg-white text-xs">Obj</Badge>
              <span className="text-xs">/20</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Badge variant="outline" className="bg-white text-xs">Theory</Badge>
              <span className="text-xs">/40</span>
            </div>
            <div className="flex items-center gap-2">
              <Calculator className="h-4 w-4 text-emerald-600" />
              <span className="text-xs font-medium">Total = CA1+CA2+Obj+Theory (/100)</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 mt-3">
            <span className="text-xs text-gray-600">Grades:</span>
            <Badge className="bg-green-100 text-green-700 text-[10px]">A: 80-100</Badge>
            <Badge className="bg-blue-100 text-blue-700 text-[10px]">B: 70-79</Badge>
            <Badge className="bg-yellow-100 text-yellow-700 text-[10px]">C: 60-69</Badge>
            <Badge className="bg-orange-100 text-orange-700 text-[10px]">P: 50-59</Badge>
            <Badge className="bg-red-100 text-red-700 text-[10px]">F: 0-49</Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-3 sm:p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div>
              <Label className="text-[10px] sm:text-xs text-muted-foreground mb-1 block">Exam</Label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
          <Card className="border-0 shadow-sm">
            <CardContent className="p-2.5 sm:p-3">
              <p className="text-[9px] sm:text-xs text-muted-foreground">Students</p>
          ...
          <Card className="border-0 shadow-sm bg-blue-50">
            <CardContent className="p-2.5 sm:p-3">
              <p className="text-[9px] sm:text-xs text-blue-600">Average</p>
            ...
            <CardHeader className="pb-3 border-b px-3 sm:px-5 pt-3 sm:pt-4">
              <div>
                ...
                <CardDescription className="text-xs">
                  {students.length} students found • Select exam to load students
                </CardDescription>
              </div>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50 dark:bg-slate-800">
                      ...
                      <TableHead className="text-center text-xs">CA1 (20)</TableHead>
                      <TableHead className="text-center text-xs">CA2 (20)</TableHead>
                      <TableHead className="text-center text-xs">Obj (20)</TableHead>
                      <TableHead className="text-center text-xs">Theory (40)</TableHead>
                      <TableHead className="text-center text-xs">Total (100)</TableHead>
                      <TableHead className="text-center text-xs">Grade</TableHead>
                      <TableHead className="text-xs">Remark</TableHead>
                      <TableHead className="text-center text-xs">Auto</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {students.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={9} className="text-center py-8">
                          ...
                          <p className="text-muted-foreground text-sm">
                            {!selectedExamId ? 'Select an exam to load students' : 'No students found for this exam'}
                          </p>
                        </TableCell>
                      </TableRow>
                    ) : (
                      students.map((student) => {
                        const entry = scoreEntries[student.id] || { ca1: '', ca2: '', objective: '', theory: '', remark: '' }
                        const ca1 = parseFloat(entry.ca1) || 0
                        const ca2 = parseFloat(entry.ca2) || 0
                        const objective = parseFloat(entry.objective) || 0
                        const theory = parseFloat(entry.theory) || 0
                          ...
                          <TableRow key={student.id} className="hover:bg-slate-50">
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={(e) => handleScoreChange(student.id, 'objective', e.target.value)}
                                className="h-8 w-16 mx-auto text-center text-xs"
                              />
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-center">
                              {total > 0 && (
                                <Badge className={cn("text-[10px]", getGradeColor(grade))}>{grade}</Badge>
                              )}
                            </TableCell>
                            <TableCell>
                              <Input
                            ...
                            <TableCell>
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleAutoFetchScores(student.id)}
          ...
          <Card className="border-0 shadow-sm overflow-hidden">
            <CardHeader className="pb-3 border-b px-3 sm:px-5 pt-3 sm:pt-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <CardTitle className="text-sm sm:text-base">Saved Scores</CardTitle>
                    ...
                    {loading ? (
                      ...
                      filteredScores.map((score) => (
                        <TableRow key={score.id}>
                          <TableCell className="font-medium text-xs">{getStudentName(score.student_id)}</TableCell>
                          ...
                          <TableCell className="text-center text-xs">{score.exam_objective_score || '-'}</TableCell>
                          <TableCell className="text-center text-xs">{score.exam_theory_score || '-'}</TableCell>
                          <TableCell className="text-center font-semibold text-xs">{score.total_score?.toFixed(1) || '-'}</TableCell>
                          <TableCell className="text-center">
                            <Badge className={cn("text-[10px]", getGradeColor(score.grade || ''))}>{score.grade}</Badge>
                          </TableCell>
                          <TableCell className="text-xs max-w-[100px] truncate">{score.remark || '-'}</TableCell>
                          <TableCell className="text-right">
                            <Button variant="ghost" size="sm" onClick={() => { setEditingScore(score); setShowEditDialog(true) }} className="h-7 w-7 p-0">
                              <Edit className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => handleDeleteScore(score.id)} className="h-7 w-7 p-0 text-red-500">
        ...
        <TabsContent value="analytics">
          <Card className="border-0 shadow-sm">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Detailed analytics available once data is collected</p>
              {stats.gradedStudents > 0 && (
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{getStudentName(editingScore?.student_id || '')}</DialogDescription>
          </DialogHeader>
          {editingScore && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs">CA1 (20)</Label>
                  <Input type="number" min="0" max="20" step="0.5"
                    value={editingScore.ca1_score || 0}
                    onChange={(e) => setEditingScore({...editingScore, ca1_score: parseFloat(e.target.value) || 0})}
                    className="h-9 text-sm" />
                </div>
                <div>
                  <Label className="text-xs">CA2 (20)</Label>
                  <Input type="number" min="0" max="20" step="0.5"
                    value={editingScore.ca2_score || 0}
                    onChange={(e) => setEditingScore({...editingScore, ca2_score: parseFloat(e.target.value) || 0})}
                    className="h-9 text-sm" />
                </div>
                <div>
                  <Label className="text-xs">Obj (20)</Label>
                  <Input type="number" min="0" max="20" step="0.5"
                    value={editingScore.exam_objective_score || 0}
                    onChange={(e) => setEditingScore({...editingScore, exam_objective_score: parseFloat(e.target.value) || 0})}
                    className="h-9 text-sm" />
                </div>
                <div>
                  <Label className="text-xs">Theory (40)</Label>
                  <Input type="number" min="0" max="40" step="0.5"
                    value={editingScore.exam_theory_score || 0}
                    onChange={(e) => setEditingScore({...editingScore, exam_theory_score: parseFloat(e.target.value) || 0})}
                    className="h-9 text-sm" />
                </div>
                <div className="col-span-2">
                  <Label className="text-xs">Remark</Label>
                  <Textarea value={editingScore.remark || ''}
                    onChange={(e) => setEditingScore({...editingScore, remark: e.target.value})}
                    rows={2} className="text-sm" />
                </div>
              </div>
              <div className="bg-slate-50 p-3 rounded-lg">
                <p className="text-sm font-medium">
                  Total: {calculateTotal(editingScore.ca1_score || 0, editingScore.ca2_score || 0, editingScore.exam_objective_score || 0, editingScore.exam_theory_score || 0)} / 100
                </p>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button className="bg-emerald-600" onClick={async () => {
              ...
              const total = calculateTotal(editingScore.ca1_score || 0, editingScore.ca2_score || 0, editingScore.exam_objective_score || 0, editingScore.exam_theory_score || 0)
