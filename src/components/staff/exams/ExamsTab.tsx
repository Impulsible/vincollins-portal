// components/staff/StaffExamsTab.tsx - COMPLETE FIXED VERSION
...
export function StaffExamsTab({ staffProfile }: StaffExamsTabProps) {
  ...
  // Subjects, Classes, and Terms for filters
  ...
  const loadExams = async () => {
    ...
    try {
      ...
      setStats({
        ...
        published: formattedExams.filter((e) => e.status === "published")
          .length,
      ...
      // Extract unique subjects, classes, and terms
  ...
  const updateExamStatus = async (
    ...
    try {
      ...
      toast.success(`Exam ${statusMessages[status]}`);
      loadExams();
    } catch (error) {
  ...
  const getTermBadge = (term: string) => {
    const termMap: Record<string, { label: string; color: string }> = {
      first: { label: "First Term", color: "text-blue-600 bg-blue-50" },
      second: { label: "Second Term", color: "text-purple-600 bg-purple-50" },
      third: { label: "Third Term", color: "text-emerald-600 bg-emerald-50" },
    };
    ...
    return (
      <Badge variant="outline" className={cn("border-0", termInfo.color)}>
                ...
                <CardContent className="p-2.5 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-base sm:text-lg md:text-xl font-bold text-green-700 dark:text-green-300">
                        {stats.published}
                      </p>
                    </div>
                ...
                <CardContent className="p-2.5 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-base sm:text-lg md:text-xl font-bold text-yellow-700 dark:text-yellow-300">
                        {stats.pending}
                      </p>
                    </div>
                ...
                <CardContent className="p-2.5 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-base sm:text-lg md:text-xl font-bold text-slate-700 dark:text-slate-300">
                        {stats.draft}
                      </p>
                    </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-slate-50 dark:bg-slate-800/50">
                          ...
                          <TableHead className="text-center text-xs hidden lg:table-cell">
                            Qs
                          </TableHead>
                          <TableHead className="text-center text-xs hidden lg:table-cell">
                            Marks
                        ...
                        {filteredExams.map((exam) => (
                          ...
                          >
                            ...
                            <TableCell className="text-xs hidden xl:table-cell">
                              {getTermBadge(exam.term || "third")}
                            </TableCell>
                            <TableCell>{getStatusBadge(exam.status)}</TableCell>
                            <TableCell className="text-xs text-gray-500 hidden 2xl:table-cell">
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-0.5 sm:gap-1">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    router.push(`/staff/exams/edit/${exam.id}`)
                                  }
                                  className="h-7 w-7 sm:h-8 sm:w-8 p-0"
                                >
                                ...
                                <DropdownMenu>
                                  ...
                                  >
                                    ...
                                    {exam.status === "draft" && (
                                      <DropdownMenuItem
                                        onClick={() =>
                                          updateExamStatus(exam.id, "pending")
                                        }
                                      >
                                    ...
                                    {exam.status === "published" && (
                                      <DropdownMenuItem
                                        onClick={() =>
                                          updateExamStatus(exam.id, "archived")
                                        }
                                      >
                  ...
                  <>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs sm:text-sm">
                            ...
                            <span className="font-semibold">
                              {selectedExam.term.charAt(0).toUpperCase() +
                                selectedExam.term.slice(1)}{" "}
                              Term
                            </span>
                          </span>
                    ...
                    <DialogFooter className="px-4 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row gap-2">
                      <Button
                        ...
                        className="h-8 sm:h-9 text-sm order-2 sm:order-1"
                      >
                        Close
                      </Button>
                      <Button
            ...
            >
              <AlertDialogContent className="max-w-[90vw] sm:max-w-md rounded-xl">
                ...
                <AlertDialogFooter className="flex flex-col sm:flex-row gap-2">
                  <AlertDialogCancel className="h-8 sm:h-9 text-sm order-2 sm:order-1">
                    Cancel
                  </AlertDialogCancel>
                  <AlertDialogAction
