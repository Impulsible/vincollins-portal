...
const moreMenuItems: NavItem[] = [
  ...
  { id: 'admin-inquiries', label: 'Inquiries', icon: MessageSquare, route: '/admin/inquiries', roles: ['admin'] },
  { id: 'admin-settings', label: 'Settings', icon: Settings, route: '/admin/settings', roles: ['admin'] },
  { id: 'admin-help', label: 'Help & Support', icon: HelpCircle, route: '/admin/help', roles: ['admin'] },
]
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shadow-lg">
        <div className="flex items-center justify-around px-2 py-1">
          {mainItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => handleNavigation(item.route, item.id)}
                className={cn(
          ...
          <Sheet open={moreMenuOpen} onOpenChange={setMoreMenuOpen}>
            <SheetTrigger asChild>
              <button className={cn("flex flex-col items-center justify-center py-1.5 px-3 rounded-xl transition-all",
                moreMenuOpen 
                  ? role === 'admin' ? "text-purple-600 dark:text-purple-400" : role === 'staff' ? "text-blue-600 dark:text-blue-400" : "text-emerald-600 dark:text-emerald-400"
              ...
              <ScrollArea className="h-full pb-20">
                <div className="p-4 border-b border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-3">
                    <Avatar className={cn("h-14 w-14 ring-2",
                      role === 'admin' ? "ring-purple-100 dark:ring-purple-900" : role === 'staff' ? "ring-blue-100 dark:ring-blue-900" : "ring-emerald-100 dark:ring-emerald-900")}>
                      <AvatarImage src={avatarUrl} alt={displayName} />
                      <AvatarFallback className={cn("bg-gradient-to-br text-white text-lg font-bold",
                        role === 'admin' ? "from-purple-600 to-indigo-600" : role === 'staff' ? "from-blue-600 to-indigo-600" : "from-emerald-600 to-teal-600")}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Badge className={cn("text-[10px] text-white mt-1",
                      ...
                      return (
                        <button key={item.id} onClick={() => handleNavigation(item.route, item.id)}
                          className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all",
                            active 
                              ? role === 'admin' ? "bg-purple-50 dark:bg-purple-950/30 text-purple-700 dark:text-purple-300"
                              : role === 'staff' ? "bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300"
                              : "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-300"
                              : "hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300")}>
                          <Icon className="h-5 w-5 shrink-0" />
                          <span className="text-sm font-medium">{item.label}</span>
                          {notificationCount > 0 && (item.id === 'notifications' || item.id === 'staff-notifications') && (
